...
class ChIDDatasetIE(Dataset):
    def __init__(self, data_path, chid_file="train_data_5w.json", idiom_file="idioms_ChID.json",
                 ...
                 max_len=300, idiom_pattern_max_length=64, is_train=True):
        cache_path = os.path.join(data_path, '{}_{}_{}_{}_IE.pkl'.format(chid_file[:-5], idiom_mask_length, max_len,idiom_pattern_max_length))
        ...
        if cached:
            ...
            self.data = {
                ...
                "idiom_candidate_pattern_token": idiom_tokenization_output["input_ids"]
                  .reshape(-1, len(processed_idiom_candidate_pattern) // len(processed_sentence), idiom_pattern_max_length),
                # 1 for real token, 0 for padding
                "idiom_candidate_pattern_mask": idiom_tokenization_output["attention_mask"]
                  .reshape(-1, len(processed_idiom_candidate_pattern) // len(processed_sentence), idiom_pattern_max_length).bool(),
                # index in candidates
...
if __name__ == "__main__":
    ...
    myData = ChIDDatasetIE("../data/", chid_file="ChID/dev_data.json", is_train=True)
