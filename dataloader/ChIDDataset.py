...
class ChIDDataset(Dataset):
    def __init__(self, path, idioms_path="../data/new_idioms.json"):
        ...
        for single_raw_data in raw_data:
            ...
            for single_idiom_index in range(single_raw_data["realCount"]):
                complete_content = complete_content.replace("#idiom#",
                                                            single_raw_data["groundTruth"][single_idiom_index], 1)

            for single_idiom_index in range(single_raw_data["realCount"]):
                single_res_data = {}
                tmp1 = content_splitter(complete_content, single_raw_data["groundTruth"][single_idiom_index], tokenizer)
                idiom_candidates = [x + "：" + idioms_dict[x] for x in single_raw_data["candidates"][single_idiom_index]]
                tmp2 = idiom_candidate_splitter(idiom_candidates, tokenizer)
                ...
                single_res_data["idiom_candidate_patterns_token"] = torch.Tensor(tmp2["idiom_candidate_patterns_token"])
                single_res_data["idiom_candidate_patterns_mask"] = torch.Tensor(tmp2["idiom_candidate_patterns_mask"])
...
if __name__ == "__main__":
    ...
    myData = ChIDDataset("../data/ChID/dev_data.json")
